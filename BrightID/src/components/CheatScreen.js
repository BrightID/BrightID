...
export class TestConnectionScreen extends React.Component {
  ...
  switchToConnection = async (connection) => {
    let { publicKey, secretKey, name } = connection;
  ...
  connectUsers = async () => {
    ...
    if (!userA && !userB) return alert('select 2 user');
    ...
    try {
      ...
      let sig1 = uInt8ArrayToB64(
        nacl.sign.detached(strToUint8Array(message), userA.secretKey),
      );
      // return alert(sig1);

      let sig2 = uInt8ArrayToB64(
        nacl.sign.detached(strToUint8Array(message), userB.secretKey),
      );
          ...
          <ScrollView>
            ...
            >
              ...
              <Button
                style={{ flex: 0 }}
                title="connect"
                onPress={() => this.goToConnectUsers()}
              />
              <View style={{ flex: 1 }} />
              <Button
                style={{ flex: 0 }}
                title="Get Info"
                onPress={() => this.testGetUserInfo()}
              />
            </View>
            <View styles={{ padding: 10 }}>
              ...
              {this.state.scope === 'info' && (
                <View
                  style={{ padding: 10, borderWidth: 1, borderColor: '#ddd' }}
                >
                    ...
                    {this.state.connections.map((connection, index) => (
                      ...
                      >
                        ...
                        <View style={{ flex: 0 }}>
                          <Button
                            title="switch"
                            onPress={() => this.switchToConnection(connection)}
                          />
                        </View>
                      </View>
                    ))}
                    ...
                    >
                      {this.state.connections.map((connection, index) => (
                        ...
                        >
                          <TouchableOpacity
                            style={{ flex: 0 }}
                            onPress={() => this.setState({ userA: connection })}
                          >
                            <Material
                              size={30}
                              name={
                    ...
                    >
                      {this.state.connections.map((connection, index) => (
                        ...
                        >
                          <TouchableOpacity
                            style={{ flex: 0 }}
                            onPress={() => this.setState({ userB: connection })}
                          >
                            <Material
                              size={30}
                              name={
