...
import { setApps, addSig, updateSig, selectAllSigs, removeAllSigs } from '@/reducer/appsSlice';
...
export const updateBlindSigs = (api) => async (dispatch: dispatch, getState) => {
  ...
  for (const app of apps) {
    ...
    for (const verification of app.verifications) {
      const sigInfo = sigs.find(sig => sig.app == app.id && sig.verification == verification);
      ...
      try {
        ...
        const uid = CryptoJS.enc.Base64.stringify(CryptoJS.lib.WordArray.random(16));
        ...
        const challenge = client.GenerateWISchnorrClientChallenge(pub, info, uid);
